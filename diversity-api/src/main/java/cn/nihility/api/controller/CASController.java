...
public class CASController {
    ...
    public String casLogin() {
        ...
        String urlEncodeService = HttpRequestUtils.urlParamsEncode(service);
    ...
    public UnifyResult<Map<String, String>> casLoginCallback(@RequestParam("ticket") String ticket) {
        ...
        URI uri = HttpRequestUtils.buildUri(url, params);
        HttpGet request = new HttpGet(uri);
        @SuppressWarnings("unchecked")
        UnifyResult<Map<String, String>> validateResultData = HttpClientUtils.executeHttpRequest(request, UnifyResult.class);
        logger.info("向 CAS 服务端校验 ticket 响应数据 [{}]", validateResultData);
        if (validateResultData == null) {
            logger.error("向 CAS 服务端校验 ticket 请求失败");
            throw new HttpRequestException(HttpStatus.UNAUTHORIZED, UnifyResultUtils.failure("向 CAS 服务端校验 ticket 请求失败"));
        } else {
            if (UnifyCodeMapping.SUCCESS.getCode().equals(validateResultData.getCode())) {
                return UnifyResultUtils.success(validateResultData.getData());
            } else {
                String message = validateResultData.getMessage();
                logger.error("向 CAS 服务端校验 ticket 异常 [{}]", message);
                throw new HttpRequestException(HttpStatus.UNAUTHORIZED, UnifyResultUtils.failure(message));
            }
        }
    }
    ...
    private void logRequestCookies(HttpServletRequest request) {
        logger.info("Request Cookies [{}]", HttpRequestUtils.cookiesToMap(request));
    }
    ...
    public String passwordLoginValidate(String username, String password,
        ...
        String serviceUrl = HttpRequestUtils.urlParamsEncode(HttpRequestUtils.addUrlParam(service, TICKET, st));
    ...
    public UnifyResult<Map<String, String>> casServerServiceValidate(@RequestParam("service") String service,
        ...
        if (StringUtils.isBlank(ticket)) {
            ...
            throw new HttpRequestException(HttpStatus.UNAUTHORIZED, UnifyResultUtils.failure("Ticket 校验票据不存在"));
        }
        ...
        return UnifyResultUtils.success(info);
    }
