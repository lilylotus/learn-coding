...
public class SnowFlakeId {
    ...
    private static Long getWorkId() {
        String hostAddress = NetUtil.getIp();
        if (log.isDebugEnabled()) {
            log.debug("host address [{}]", hostAddress);
        }
        return stringEncodeToLong(hostAddress);
    }
    ...
    private static Long getDataCenterId() {
        ...
        return stringEncodeToLong(hostName);
    }

    private static Long stringEncodeToLong(String code) {
        if (StringUtils.isBlank(code)) {
            // 如果 code 为空，则使用随机数备用
            return RandomUtils.nextLong(0, 31);
        }
        int[] ints = StringUtils.toCodePoints(code);
        int sums = 0;
        for (int i : ints) {
            sums += i;
        }
