...
public final class SnowflakeIdWorker {
     ...
     * 起始的时间戳 (2015-01-01)
     */
    private static final long START_TIME_STAMP = 1420041600000L;
    ...
    /**
     * 机器标识占用的位数 10 = workerId + dataCenterId ，工作 ID 占用位数
     */
     ...
     * 最大的 worker 数值， 31
     */
    private static final long MAX_WORKER_ID_NUM = ~(-1L << WORKER_ID_BIT);
    /**
     * 最大的序列号值 4095
     */
     ...
     * 机器 ID 向左移动 12 位
     */
    private static final long WORKER_ID_LEFT = SEQUENCE_BIT;
    /**
     * 数据中心 ID 向左移动 17 位， (12 + 5)
     */
    private static final long DATA_CENTER_LEFT = WORKER_ID_LEFT + WORKER_ID_BIT;
    /**
     * 时间戳向左移动 22 位 (12 + 5 + 5)
     */
    ...
    public synchronized long nextId() {
        ...
        if (currTimeStamp < lastTimestamp) {
            throw new IllegalArgumentException("Clock moved backwards. " +
                "Refusing to generate id for " + (lastTimestamp - currTimeStamp) + " milliseconds .");
        }
    ...
    public static long nextSnowId() {
        ...
        if (idWorker == null) {
            synchronized (SnowflakeIdWorker.class) {
                ...
                if (idWorker == null) {
                    idWorker = new SnowflakeIdWorker(getDataCenterId(), getWorkId());
                }
